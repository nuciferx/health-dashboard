...
## Update: 2026-07-30 — ปรับเทเปอร์ 9 วันสุดท้าย (หลังวิเคราะห์ session 29 ก.ค.)

### สภาพจริงที่อ่านได้
- **Oura 30 ก.ค.** (จาก log GitHub Actions run 30515123901 — ดึง API ตรงไม่ได้ใน session นี้): นอน 5.2 ชม. 🔴 · deep 0.7 / REM 0.7 🔴 · HRV 20 ↓ · RHR 70 ↑ · **readiness 49 🔴** · resilience limited
- **Strava 29 ก.ค.** (`19512035511`): TrailRun **21.0 กม. / +1,255 ม.** · elapsed 8:44 · avg HR 151 · max 179 · 2,825 kcal · relative effort 450
- ทำจริงเป็น mini race sim (แผนไว้ 25 ก.ค. ที่ 15-18 กม./+800-1,000) ช้า 4 วัน และ **ใหญ่กว่าแผน**

### วิเคราะห์ stream/lap (คำนวณจาก 21 lap)
| ตัวชี้วัด | ค่า | อ่านว่า |
|---|---|---|
| HR ครึ่งแรก → ครึ่งหลัง | **153.3 → 146.5 (−6.7)** | ✅ **ไม่มี cardiac drift** — เป้า "ไม่ถล่มที่ชั่วโมงที่ 3" พิสูจน์แล้วที่ 8 ชม. |
| เวลาในกิโลที่ avg HR >150 | 4.52 ชม. = **56%** | ❌ เกินกฎ ≤150 |
| กิโลขาขึ้น (9 กม.) | 3.50 ชม. · avg HR **157.3** | ❌ ขาขึ้นเร็วเกิน |
| กิโลขาลง (9 กม.) | 3.06 ชม. · avg HR 143.3 | ✅ |
| ยืนพัก | **43 นาที** (lap 17: elapsed 3,664 / moving 1,098) | ❌ = 8% ของเวลา |
| vertical density | **60 ม./กม. = เท่าสนามแข่งเป๊ะ** | → เป็นตัวทำนายเพซที่ดีที่สุดที่มี |

**เพซปรับ vertical** (eff-กม. = ระยะ + ไต่×8): 29 ก.ค. = **3.9** · ต้องได้เพื่อ cutoff 13:00 = **4.5** · เพื่อ 12:00 = **4.9**
→ ยังห่างเพซ cutoff ~13% · เทียบ 14 มิ.ย. (22.8 กม./+892) = 4.8 eff-กม./ชม. แต่พังจากกินน้อย
→ **ข้อสรุป: ความทนพิสูจน์แล้ว ที่ขาดคือเพซ+ความสด ไม่ใช่ volume**

### แก้อะไร
- **ตัด long session 1 ส.ค. จาก 4-5 ชม. → DRESS REHEARSAL 2-2.5 ชม.** (vertical +300-500) — เหตุ: 8:44/+1,255 เก็บไปแล้ว 29 ก.ค. · ทำ 4-5 ชม. อีกที่ 7 วันก่อนแข่ง = แลกความสดโดยไม่ได้ฟิตเพิ่ม + เสี่ยงเอ็น peroneal (ประวัติ มิ.ย.)
- **ใส่ uphill economy 2 ครั้ง** (3 ส.ค. 40 นาที · 5 ส.ค. 25-30 นาที) — ลู่ชัน 8-12% คุม HR **นิ่ง 140-148** เป้า "เร็วขึ้นที่ HR เดิม" ไม่ใช่เหนื่อยขึ้น
- **เขียนแผน 2-7 ส.ค. เป็นรายวัน** — เดิมเป็น range fallback ก้อนเดียว (`iso >= '2026-08-02' && <= '2026-08-07'`) ตอนนี้ลบ fallback ทิ้ง ใส่ 6 วันตรง ๆ ใน `W7_BY_DATE`/`W7`
- **เพิ่มวินัยจุดหยุดเป็นเป้าวัดผล** — ซ้อม 1 ส.ค. ยืนรวม ≤5 นาที · วันแข่ง ≤20 นาที (เลเวอร์ที่ได้เยอะสุด/ฟรีสุด ~25 นาที)
- **แก้ pacing วันแข่ง:** 10 กม.แรกจาก "≤150" → **≤140** · ตั้งเป้าให้ตรงความจริง (12:30-13:00 คือเป้าหลัก · 12:00 คือ upside · ห้ามออกตัวด้วยเพซ 12:00)

### ไฟล์ที่แตะ
- `morning_digest.py` — `W7_BY_DATE` รายวันครบ 28 ก.ค.-8 ส.ค. · ลบ range fallback 08-02..08-07 ใน `plan_for()`
- `cf-worker/src/index.js` — `W7` + `planFor()` sync ตรงกัน (`/plan` `/today` `/week`)
- `CM6_i2_ตารางซ้อม.md` — ตาราง W7 ใหม่ + กฎห้าม/กฎปรับอัตโนมัติ + ตาราง "ตัวเลขจริงจาก 29 ก.ค." ใต้ pacing

### TEST — PASS
- `node --check cf-worker/src/index.js` ✅
- `plan_for()` python: ครบทุกวัน 30 ก.ค.-8 ส.ค. ไม่มีวันไหนคืน `None` ✅
- `planFor()` worker (extract → require): ผลตรงกับ python ทุกวัน ✅
- ⚠️ `python morning_digest.py` เต็มรูปแบบรันไม่ได้ใน session นี้ (ไม่มี `OURA_TOKEN` + network policy บล็อก `api.ouraring.com`) — ทดสอบด้วย import + เรียก `plan_for()` แทน

### Known gaps / next action
- [ ] **ยังไม่ deploy worker** — ต้อง `cd cf-worker && npx wrangler deploy` (pre-release gate: ขออนุมัติก่อน) · จนกว่าจะ deploy `/plan` ในบอทยังโชว์แผนเก่า (digest 6 โมงใช้ `morning_digest.py` จาก repo → อัปเดตทันทีที่ push)
- [ ] **cron digest ครอบไม่พอ** — 30 ก.ค. รอบ 06:49 กับ 08:54 ICT ข้าม (Oura ยังไม่ sync) รอบที่ส่งจริง 11:54 ICT **ไม่ตรง cron ไหนเลย** (cron หมด 09:30 ICT) = GitHub delay ช่วยไว้ → ควรเพิ่มรอบถึง ~11:00 ICT
- [ ] **`/oura/*` บน worker ไม่มี auth** — proxy ด้วย `env.OURA_TOKEN` โดยไม่ตรวจอะไร ใครรู้ URL ก็อ่านข้อมูลสุขภาพได้ (ต่างจาก `/telegram` ที่มี `TELEGRAM_WEBHOOK_SECRET`)

---

## Last updated: 2026-07-30 — ปรับเทเปอร์ 9 วันสุดท้าย: ตัด long 1 ส.ค. → dress rehearsal + uphill economy ×2 + แผน 2-7 ส.ค. รายวัน; prior: 27 ก.ค. ใส่ time-on-feet 4-5 ชม. + deploy worker (388b711a)
