...
## Update: 2026-08-01 (d) — จับคู่ "กินอะไร → HR ทำอะไร" จาก 29 ก.ค. (ผู้ใช้เล่าเอง)

**ผู้ใช้เล่า:** 29 ก.ค. กม.4 ลงไปกินโค้ก กลับขึ้นมาเหนื่อย HR สูง · กม.8 กินกาแฟ+กล้วยตาก 2 ชิ้น ไป กม.10-12 สบาย · กม.12-14 กินข้าว แย่อีก
**ตรวจกับ lap data (Strava 19512035511) → ตรงทุกจุด:**

| กิน | กิโลไต่ถัดมา | อัตราไต่ | HR |
|---|---|---|---|
| (ยังไม่กิน) กม.1-2 | ไต่ 11% | 402 / 362 ม./ชม. | 144 / 156 |
| โค้ก @กม.4 | กม.5-6 | **207** / 320 | 155 / **165** |
| กาแฟ+กล้วยตาก @กม.8-9 | กม.10-11 | **319** / 262 | 157 / 158 |
| ข้าว @กม.12-14 | กม.14 | **251** | 158 |

กม.5 หลังโค้ก = 35:39 นาที ช้าที่สุดของวัน · กม.6 HR 165 สูงสุดของวัน
**กลไก:** splanchnic steal (เลือดถูกดึงไปกระเพาะ = ขาได้น้อยลง = HR ชดเชย) · โค้กเข้มข้น ~11% เกินช่วงดูดซึมดี 6-8% → ค้างกระเพาะ + ดึงน้ำเข้าลำไส้
**เพิ่มเข้าการ์ดวันแข่ง:** section "🍌 กินยังไงไม่ให้ HR พุ่ง" — น้อยแต่บ่อย 70-100 kcal ทุก 15-20 นาที · กล้วยตากเอาไปเอง · ห้ามมาม่า/ข้าวที่ CP · โค้กเฉพาะหลัง CP3 ผสมน้ำ · กินตอนราบ/ทางลง
**+ พบว่ายืนนิ่งเยอะมาก:** กม.17 หยุดจริง 43 นาที · กม.9/13 ใช้ 38/32 นาทีบนทางราบ → ตั้งกฎอยู่ใน CP ≤5 นาที

## Update: 2026-08-01 (e) — ผู้ใช้ยืนยัน "รอดเพราะขนมปังขาว" ที่แม่สลอง

**ผู้ใช้เล่า:** แม่สลอง (16 พ.ย. 2024) รอดเพราะ **ขนมปังขาว** — สนามนี้คือสนามที่จบด้วย avg HR ต่ำสุด (149)
**ตรวจ lap แม่สลอง (Strava 12912836285):** 36.93 กม./+1,917 ม./10:40/avg HR 149.3/max 183 · ครึ่งแรก HR 156-172 · **ครึ่งหลัง (กม.22-37) ลงมาอยู่ 128-147 แล้วอยู่ตรงนั้นจนจบ** ยังไต่ได้ 244-280 ม./ชม. ที่ HR 141-147 = fade แบบคุมได้ ไม่ใช่ collapse (ต่างจาก CM6 2024 ที่ HR ร่วง 120-130 พร้อมอัตราไต่ตกเหลือ 92 ม./ชม.)

**สรุปกฎอาหารที่อธิบายได้ทุกกรณี: ไขมันต่ำ + กากใยต่ำ + ปริมาณน้อย = ผ่าน**

| ผ่าน ✅ | พัง ❌ |
|---|---|
| ขนมปังขาว (ไขมัน ~0 กากใย ~0 = กระเพาะระบายเร็วสุดในของแข็ง) | โค้ก (เข้มข้น 11% > ช่วงดูดซึม 6-8%) |
| กล้วยตาก (น้ำตาลเชิงเดี่ยว ปริมาณน้อย) | ข้าว+กับข้าว (**ไขมันคือเบรกกระเพาะที่แรงสุด**) |
| กาแฟ (คาเฟอีน) | มาม่า (เส้นทอด=ไขมัน + น้ำซุปเยอะ) |

**เพิ่มเข้าการ์ดวันแข่ง:** ตารางของผ่าน/พัง + แผนกินจริง — ขนมปังขาว 2 แผ่น (150 kcal) + กล้วยตาก 2 ชิ้น (75 kcal) = **~225 kcal/ชม.** · พก 12 แผ่น ≈ 900 kcal ≈ 300 ก. · **CP ไม่มีขนมปังขาว ต้องพกเอง** (CP มีแค่ผลไม้/ขนม/มาม่า/กาแฟ/Pocari/น้ำ)

## Update: 2026-08-01 (f) — แผนกิน-นอน 8 วันสุดท้าย (1-8 ส.ค.)

**ผู้ใช้ขอ:** วางแผนการกิน-นอนตั้งแต่วันนี้ → สร้าง **`CM6_i2_แผนกินนอน_8วันสุดท้าย.md`**

**การตัดสินใจสำคัญ: ยกเลิกเป้าลดน้ำหนัก 90-91 กก.** — คำนวณแล้ว โหลดคาร์บทำให้หนักขึ้น ~2 กก. แต่คิดเป็นพลังงานยกตัวขึ้น +2,483 ม. = **~50 kcal ตลอดสนาม** ขณะที่ได้ไกลโคเจนเพิ่ม ~2,000 kcal = **คุ้ม 40 เท่า** · และ DNF 2024 เกิดจากไกลโคเจนหมด (HR ร่วง 120 ที่ กม.28-33) ไม่ใช่น้ำหนักเกิน
→ ใส่ ⛔ หัวไฟล์ `CM6_i2_เมนูอาหาร.md` ว่าอย่าใช้ช่วง 1-8 ส.ค. (เมนูนั้นขาดดุล 500 kcal = ทำร้ายตอนนี้)

**นอน (เลเวอร์ใหญ่สุดที่เหลือ):** จาก 5.3 ชม. → ตาราง 7.5 ชม./คืน เลื่อนเวลานอนเร็วขึ้นวันละ 15 นาที เพื่อไปจบที่ตื่น 03:30 วันแข่ง
- **คืนสำคัญที่สุด = พฤ 6 ส.ค. (2 คืนก่อนแข่ง)** ไม่ใช่คืนก่อนแข่ง (คืนก่อนแข่งนอนไม่ดีเป็นเรื่องปกติ)
- กาแฟก่อนเที่ยงเท่านั้น → ได้ 2 เด้ง: นอนดีขึ้น + วันแข่งคาเฟอีนออกฤทธิ์แรงกว่าเดิม
- ห้ามเหล้า (ทำลาย deep sleep ที่ Oura flag อยู่) · ตื่นมาเจอแสงทันที (เลื่อนนาฬิกาชีวิต)

**กิน 3 เฟส:** 1-4 ส.ค. maintenance 2,600-2,800 kcal (หยุดขาดดุล) · 5-6 ส.ค. โหลดคาร์บ 500-600 ก./วัน · 7 ส.ค. กากใยต่ำ+ไขมันต่ำ+ของคุ้นเคย มื้อเย็นเสร็จก่อน 18:00
**เช้าวันแข่ง:** ตื่น 03:30 · 03:45 ขนมปังขาว 3-4 แผ่น + แยม + กล้วย + กาแฟดำ (~500-600 kcal) · ระหว่างแข่ง 225 kcal/ชม.
**การบ้าน:** ซ้อมมื้อเช้าวันแข่ง 4 หรือ 5 ส.ค. (กินชุดเดียวกันแล้วเดิน 60-90 นาที)

## Update: 2026-08-01 (g) — เมนูจริงรายวัน 8 วันสุดท้าย + ลิสต์ซื้อของ

**ผู้ใช้ขอ:** "จัดเมนู" (ต่อจากแผนกิน-นอน) → สร้าง **`CM6_i2_เมนู_8วันสุดท้าย.md`** = ระดับ "กินอะไรจริง ๆ" มื้อต่อมื้อ พร้อมคาร์บ/โปรตีนต่อมื้อ + เมนูสลับ + ลิสต์ซื้อของ

**สร้างรอบของ 3 อย่างที่พิสูจน์แล้ว: ขนมปังขาว · กล้วยตาก · กาแฟ** (ไม่เอาของใหม่มาเสี่ยงในสัปดาห์สุดท้าย)
- เฟส 1 (1-4 ส.ค.): ~2,800 kcal · คาร์บ ~348 ก. · โปรตีน ~146 ก. — **ปรับเป้าคาร์บจาก 400-450 → ~350** ให้ตรงกับเมนูที่กินได้จริง (แก้ในไฟล์แผนด้วย)
- เฟส 2 (5-6 ส.ค.): ~528 ก. คาร์บ · เย็นเสร็จก่อน 18:30 · ชาบูกินได้ครั้งสุดท้าย พุธ 5 ส.ค.
- เฟส 3 (7 ส.ค.): ~506 ก. กากใยต่ำ · **ระบุชัดว่าชาบู (ผักกาดขาว/เห็ดเข็มทอง/ผักบุ้ง/สาหร่าย) = ห้าม** เพราะกากใยสูง
- วันแข่ง: 03:45 ขนมปังขาว 4 แผ่น+แยม+กล้วย+กาแฟ (~600 kcal) · ระหว่างแข่ง 225 kcal/ชม.

**ลิสต์ซื้อของ:** ขนมปังขาว 3 แถว (~55 แผ่น) · กล้วยตาก ~50 ชิ้น (พกแข่ง 24) · นมจืด 14 กล่อง · Pocari 6-8 ซอง ฯลฯ
**บริบทที่ใช้ตัดสิน:** ผู้ใช้ส่งรูปชาบูมื้อเย็น 1 ส.ค. → ตอบว่าดีสำหรับเฟส 1 แต่คาร์บน้อย ต้องสั่งข้าว/เส้นเพิ่ม และมื้อแบบนี้ห้ามวันที่ 7 ส.ค.
